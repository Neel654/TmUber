...
public class CityMap
{
  ...
  // A new method is created to get zone of an address according to the instructions
  // It returns -1 if the address is not valid
  public static int getCityZone(String address) {
    if (!validAddress(address)) {
      return -1;
    } else {
      int[] blck = getCityBlock(address);

      if (0 < blck[0] & blck[0] < 6) {
        if (5 < blck[1] & blck[1] < 10) {
          return 0;
        } else {
          return 3;
        }
